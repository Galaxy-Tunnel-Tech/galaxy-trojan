    ...
    try {
        ...
        if (response.ok) {
            ...
            const fetchedIPs = text.split("\n")
                .map(line => line.trim())
                .filter(line => line.length > 0 && !line.startsWith("#") && /^[a-zA-Z0-9.:-]+$/.test(line) && !isPrivateOrLocalAddress(line))
                .slice(0, 64);
...
var worker_default = {
    async fetch(request, env = {}, ctx) {
        ...
        dohURLs = String(env.DNS_RESOLVERS || "").split(",").map(v => v.trim()).filter(Boolean);
        if (!dohURLs.length) dohURLs = [dohURL, "https://dns.google/dns-query", "https://dns.quad9.net/dns-query"];
        ...
        if (upgradeHeader === "websocket") {
            if (requestedPath !== runtimeWSPath) return new Response("Not found", { status: 404 });
            return await proxyOverWSHandler(request, allowedUUIDList, allowedTrojanHashes, runtimeWSPath);
        }
...
function processProxyHeader(buffer, allowedUUIDs, allowedTrojanHashes) {
    ...
    if (uint8.byteLength >= 58 && uint8[56] === 0x0D && uint8[57] === 0x0A) {
        ...
        if (uint8.byteLength < 60) return { hasError: true, message: "Truncated Trojan address header" };
        ...
        switch (addressType) {
            case 1: // IPv4
                ...
                break;
            case 3: // Domain
                if (uint8.byteLength < addressValueIndex + 1) return { hasError: true, message: "Truncated Trojan domain length" };
                addressLength = uint8[addressValueIndex];
                addressValueIndex += 1;
                if (uint8.byteLength < addressValueIndex + addressLength + 4) return { hasError: true, message: "Truncated Trojan domain" };
        ...
        if (uint8.byteLength < portIndex + 4) return { hasError: true, message: "Truncated Trojan port" };
        const portRemote = new DataView(uint8.buffer, uint8.byteOffset + portIndex, 2).getUint16(0);
        if (uint8[portIndex + 2] !== 0x0d || uint8[portIndex + 3] !== 0x0a) return { hasError: true, message: "Invalid Trojan final CRLF" };
...
// ============================================
async function proxyOverWSHandler(request, allowedUUIDs, allowedTrojanHashes, expectedPath = DEFAULT_WS_PATH) {
    const requestPath = new URL(request.url).pathname.replace(/^\/+|\/+$/g, "");
    if (requestPath !== expectedPath) return new Response("Not found", { status: 404 });
    ...
    readableWebSocketStream.pipeTo(new WritableStream({
        async write(chunk, controller) {
            ...
            if (remoteSocketWrapper.value || headerParsed) {
                const writer = remoteSocketWrapper.value?.writable?.getWriter();
                if (writer) { await writer.write(incoming); writer.releaseLock(); }
                return;
            }

            const merged = new Uint8Array(headerBuffer.length + incoming.length);
            merged.set(headerBuffer, 0); merged.set(incoming, headerBuffer.length);
            headerBuffer = merged;
            if (headerBuffer[0] !== 0 && headerBuffer.length < 58) return;
            if (headerBuffer[0] === 0 && headerBuffer.length < 24) return;
            if (headerBuffer.length >= 58 && headerBuffer[56] === 0x0d && headerBuffer[57] === 0x0a && headerBuffer.length < 64) return;
            let result = processProxyHeader(headerBuffer, allowedUUIDs, allowedTrojanHashes);
            ...
            headerParsed = true;
            const rawClientData = headerBuffer.slice(rawDataIndex);
...
async function handleUDPOutBound(client, responseHeader, log) {
    ...
    transformStream.readable.pipeTo(new WritableStream({
        async write(chunk) {
            let dnsQueryResult = null;
            let resolverError = null;
            for (const resolver of dohURLs) {
                try {
                    ...
                    if (resp.ok) { dnsQueryResult = await resp.arrayBuffer(); break; }
                    resolverError = new Error(`DoH ${resp.status}`);
                } catch (e) { resolverError = e; }
            }
            if (!dnsQueryResult) throw resolverError || new Error("All DoH resolvers failed");
